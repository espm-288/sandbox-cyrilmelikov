...
neon_download(product = "DP1.00094.001", site = site_names, type = "basic")
neon_download(product = "DP4.00200.001", site = site_names, type = "basic")

neon_store(product = "DP1.00094.001", table="SWS_30_minute")
neon_store(product = "DP4.00200.001", table="nsae-basic")

flux_data <- neon_table(table = "nsae-basic")

neon_index(table = "nsae")%>% count(site)

## Soil water data, we don't need this right now but here's
## a remote connection to it.
con <- neon_db()
SWS_data <- tbl(con, "SWS_30_minute-basic")

print(SWS_data,)

ggplot(SWS_data, aes(x=time, y= vswc))

flux_data <- flux_data %>% 
  mutate(time = as_datetime(timeBgn))

tbl(con, "SWS_30_minute-expanded") %>% colnames()

flux_data %>% count(siteID)
...
co2_data %>% filter(siteID=="BART")

flux_data %>% ggplot(aes(x= time))+ geom_histogram()
...
SRERF30<-flux_target_30m %>% filter(siteID=="SRER")
ggplot(SRERF30, aes(x=time, y=nee))+geom_point()

BARTF30<-flux_target_30m%>% filter(siteID=="BART")
ggplot(BARTF30, aes(x= time, y=nee))+ geom_point()

#This calculates the average variation in nee  between 2 consecutive months at each site across the 4 years : This will be the drift we will be adding to the model 
drift <- flux_target_30m %>% 
  mutate(month = month(time), year = year(time), day= day(time))%>%
  group_by(siteID, year, month) %>%
  summarise(mean_nee = mean(nee, na.rm=TRUE),
            mean_le = mean(le, na.rm=TRUE)) %>%
  mutate(diff_nee =  mean_nee - lag(mean_nee))%>%
  group_by(month,siteID)%>%
  summarise(avg_diff_consec_monthl=mean(diff_nee, na.rm= TRUE))
 
drift
# This calculates the average variations in latent heat between 2 consecutive months at each site across the 4 years
flux_target_30m %>% 
  mutate(month = month(time), year = year(time), day= day(time))%>%
  group_by(siteID, year, month) %>%
  summarise(mean_nee = mean(nee, na.rm=TRUE),
            mean_le = mean(le, na.rm=TRUE)) %>%
  mutate(diff_le =  mean_le - lag(mean_le))%>%
  group_by(month,siteID)%>%
  summarise(new_col=mean(diff_le, na.rm= TRUE))



#This calculates the average nee on a particular day at a particular site over the 4 years period 
flux_target_30m %>% 
  mutate(month = month(time), year = year(time), day= day(time))%>%
  group_by(month, day, siteID)%>%
  summarise(new_col=mean(nee, na.rm= TRUE))
...
filter(year==2017)%>%
  mutate(diff_nee_2017= diff_nee-0)%>%
  filter(year==2018)%>%
  mutate(diff_nee_2018= diff_nee-0)
  ...
  mutate(diff_nee_BART= filter(siteID=="BART", month==3))


  
  
filter(siteID=="BART", month==3)%>%
  
   group_by(siteID,year(2017))
  
  mutate(mean_diff_2017= lag(diff_nee))
...
write.csv(BARTF30,"BARTF30.csv")
BART30min<-read.csv("BARTF30.csv", na.strings = c("", "NA"))
BART30min[complete.cases(BART30min), ]

BARTF30A<-BARTF30[complete.cases(BARTF30),]
ggplot(BARTF30A,aes(x=time, y=nee))+geom_point()flux
...
SRERFD <-flux_target_daily %>% filter(siteID=="SRER")
ggplot(SRERFD, aes(x=time, y=nee))+geom_point() 

 flux_target_daily %>% filter(time=="")

BARTFD<-flux_target_daily %>% filter(siteID=="BART")
ggplot(BARTFD, aes(x=time, y=nee))+geom_point() 

BARTFDA<-BARTFD[complete.cases(BARTFD),]
BARTFDA
  
lm(nee~time, data = BARTFD)
summary(lm(nee~time, data = BARTFD))
...
sm30 <- neon_table(table = "SWS_30_minute-basic")
