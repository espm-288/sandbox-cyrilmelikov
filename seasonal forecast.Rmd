...
```{r}
ARIMA_nee_saved <- ARIMA_nee_saved %>% 
  pivot_longer(c(`mean_nee`, `sd_nee`),
               names_to = "nee_statistics", 
               values_to = "nee", 
               names_repair = "unique")
...
```{r}
ARIMA_swvc_saved <- ARIMA_swvc_saved %>%
  pivot_longer(c(`mean`, `sd`),
               names_to = "vswc_statistics",
               values_to = "vswc", 
               names_repair = "unique")
```
...
######Draft things after this point 

#Soil Water Table
```{r}
db = neon_db()
swvc = tbl(db, "SWS_30_minute-expanded") %>% select(time = startDateTime, siteID = siteID, swvc = VSWCMean) %>% collect()
```

#Soil water forecast(30min)

```{r}
swvc30<- as_tsibble(sm30_target,index = time, key = siteID)
```
```{r}
swvc30 %>% fill("vswc", .direction = "up") %>% 
  model(
  ets=ETS(vswc),
  arima=ARIMA(vswc),
  snaive=SNAIVE(vswc)
...
```{r}
forecast
glance(forecast) %>% arrange(AICc)
```

#Daily forecast
```{r}
targets %>% fill(c("nee", "le", "vswc")) %>% 
  model(
  ets = ETS(nee),
  arima = ARIMA(nee),
  snaive= SNAIVE(nee)
